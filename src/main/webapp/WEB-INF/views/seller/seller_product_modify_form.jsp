        ...
        <script>
            window.onload = function () {
                ...
                if (freeShipping.value !== "") {
                    let value = freeShipping.value.replace(/[^0-9]/g, "");

                    if (value !== "") {
                        let commaValue = Number(value).toLocaleString();
                        freeShipping.value = commaValue;
                        freeShippingText.innerText = commaValue;
                    }
                }

                freeShipping.addEventListener("input", function () {
            ...
            function delImgL() {
                let imageLDiv = document.getElementById("image_l_div");
                let oriImageL = document.getElementById("ori_image_l");

                oriImageL.value = "no_file";
                imageLDiv.style.display = "none";
            }

            function delImgS() {
                let imageSDiv = document.getElementById("image_s_div");
                let oriImageS = document.getElementById("ori_image_s");

                oriImageS.value = "no_file";
            ...
            function send(f) {
                ...
                const oriImageL = document.getElementById("ori_image_l").value;
                const oriImageS = document.getElementById("ori_image_s").value;

                if (oriImageL === "no_file" && imageL.value === "") {
                    alert("대표 이미지를 등록해주세요.");
                    imageL.focus();
                    return;
                }

                if (oriImageS === "no_file" && imageS.value === "") {
                ...
                const freeShipping = document.getElementById("free_shipping");
                freeShipping.value = freeShipping.value.replace(/,/g, "");

                let formData = new FormData(f);

                fetch("/seller_product_modify.do", {
                    method: "post",
                    body: formData
                })
                .then(res => res.json())
                .then(data => {
                    if (data.result == 1) {
                        ...
                        alert("상품 수정에 실패했습니다. 관리자에게 문의 바랍니다.");
                    }
                });
            }
        </script>
